...
// TODO: Conversion vs Casting
In C#, conversion and casting are two ways to change the type of a value from one data type to another. 

- **Conversion**: Involves changing the data from one type to another implicitly or explicitly, typically between compatible types.

- **Casting**: Explicitly changes the type of a value from one type to another, potentially risking data loss or exceptions if the conversion is not valid.
...
- Explicit Conversion: Involves explicitly specifying the type conversion using conversion operators or methods like `Convert.ToX()` or type casts `(type)`.
  ```csharp
...
- Explicit Type Cast: Involves explicitly specifying the desired type in parentheses.
  ```csharp
  double numDouble = 10.5;
  int numInt = (int)numDouble; // Casting double to int
  ```

Both conversion and casting are essential concepts in C# for handling data of different types, but they serve different purposes and should be used appropriately depending on the situation.
...
// TODO: Includes using literals like 0L, instead of 0.
